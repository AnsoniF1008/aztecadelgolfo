...
export default function Gallery() {
  ...
  return <section className="sec"><div className="wrap">
    <div className="head"><div><h2>Club gallery</h2><p>Photos and videos from our members. Every post is reviewed before it shows up here.</p></div><Link className="btn" to="/upload">Upload photo or video</Link></div>
    ...
    {loading ? <Loading /> : items.length ? <div className="grid">{items.map(m => <MediaCard key={m.id} m={m} />)}</div> : <Empty>Nothing here yet. <Link to="/upload">Be the first to post.</Link></Empty>}
    {!done && !loading && <p style={{ marginTop: 28 }}><button className="btn gray" onClick={more}>Load more</button></p>}
  </div></section>
}
