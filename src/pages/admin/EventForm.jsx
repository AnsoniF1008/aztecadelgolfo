...
export default function AdminEventForm() {
  ...
  const [f, setF] = useState(isNew ? BLANK : null); const [img, setImg] = useState(null); const [err, setErr] = useState(null); const [signups, setSignups] = useState([])
  useEffect(() => {
    if (isNew) return
  ...
  const save = async e => {
    ...
    try {
      ...
      if (!f.startsAt) throw new Error('The start date is missing.')
  ...
  return <>
    ...
    <form className="form wide" onSubmit={save}>{err && <p className="err">{err}</p>}
      ...
      <div className="actions-row"><button className="btn">Save</button><Link className="btn gray" to="/admin/events">Cancel</Link>{!isNew && <button type="button" className="btn red" onClick={remove}>Delete event</button>}</div>
    </form>
